...
export function Hero() {
  ...
  const clipRight = useTransform(smoothProgress, [0, 0.6], [100, 0]);
  ...
  const dividerLeft = useTransform(smoothProgress, [0, 0.6], [0, 100]);
  const dividerLeftPx = useTransform(dividerLeft, (v) => `${v}%`);
  const textY = useTransform(smoothProgress, [0, 1], [0, -30]);
  const beforeLabelOpacity = useTransform(smoothProgress, [0, 0.15, 0.5], [1, 0.6, 0]);
  const afterLabelOpacity = useTransform(smoothProgress, [0.3, 0.6], [0, 1]);
